...
import com.sparta.myselectshop.repository.ApiUserTimeRepository;
...
@Slf4j(topic = "UseTimeAop") //로그
@Aspect //@Aspect 어노테이션을 설정해야 Aop 사용이 가능
@Component //bean class에만 적용 가능해서 @Component 어노테이션을 달아둔다.
public class UseTimeAop {

    private final ApiUserTimeRepository apiUserTimeRepository;

    public UseTimeAop(ApiUserTimeRepository apiUserTimeRepository) {
    ...
    public Object execute(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (auth != null && auth.getPrincipal().getClass() == UserDetailsImpl.class) {
                ...
                // API 사용시간 및 DB 에 기록
                ApiUseTime apiUseTime = apiUserTimeRepository.findByUser(loginUser).orElse(null);
                if (apiUseTime == null) {
                    // 로그인 회원의 기록이 없으면
